...
def generate(image_block: Image.Image, crop_chk:bool):
    ...
    return [croped_img_dir, os.path.join(sub_dir, "proj.mp4"), os.path.join(sub_dir, "PTI_render", "pre.mp4"), os.path.join(sub_dir, "PTI_render", "post.mp4")]
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title=_TITLE, theme=gr.themes.Soft()) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                crop_image_block = gr.Image(type='pil', image_mode='RGB', height=290, label='Cropped image', tool=None, aspect_ratio='auto')
        ...
        with gr.Row(variant='panel'):
            ...
            img_run_btn.click(check_img_input, inputs=[image_block], queue=False).success(
                ...
                outputs=[crop_image_block, proj_video_block, pre_video_block, post_video_block]
            ).then(
                lambda results: {
                    ...
                    'proj_video_block': results[1] if results and results[1] is not None else 'No proj video generated',
                    'pre_video_block': results[2] if results and results[2] is not None else 'No pre video generated',
                    'post_video_block': results[3] if results and results[3] is not None else 'No post video generated',
